...
type ExpenseLocationRepository interface {
	...
	DeleteExpenseLocation(id, uid string) error
...
type IncomeTypeRepository interface {
	...
	UpdateIncomeType(uid string, incomeType model.IncomeType) (model.IncomeType, error)
	DeleteIncomeType(id, uid string) error
