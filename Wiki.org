...
- Authentication > Emails > SMTP。内蔵メールサービスはテスト用途で実ユーザーには
  届かないため、本番では Resend / SES 等を接続する。
  ドメイン検証とテンプレート日本語化まで含めた手順は README.org の TODO を参照
- (=app_config= の強制アップデート判定はリリースごとの手動設定を必要としない。
  非常ブレーキとして使うときだけ =just require-app-version= を実行する。
  README.org の「強制アップデートの非常ブレーキ」を参照)
